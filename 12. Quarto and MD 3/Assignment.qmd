...
episodes_np <- nrow(season_data)
avg_viewers <- mean(season_data$viewers)
popular_ep <- season_data$title[which.max(season_data$viewers)]
descr <- season_data$description[which.max(season_data$viewers)]
...
Season `r params$season` of Game of Thrones consisted of `r episodes_np` episodes that aired between April 17 and June 19, 2011 on HBO. The show gathered an average of `r avg_viewers` million first-day TV viewers in the US, with the number growing from 2.22 to 3.04 million by the end of the season.

The most popular episode of the season was `r popular_ep`, in which:
