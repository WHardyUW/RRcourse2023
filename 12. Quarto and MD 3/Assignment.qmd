...
first_date <- season_data[1,6]
last_date <- season_data[nrow(season_data),6]
first_viewers <- season_data[1,7]
last_viewers <- season_data[nrow(season_data),7]
```
### Season `r params$season` summary

Season `r params$season` of Game of Thrones consisted of `r episodes_no` episodes that aired between `r first_date` and `r last_date` on HBO. The show gathered an average of `r avg_viewers` first-day TV viewers in the US, with the number growing from `r first_viewers` to `r last_viewers` million by the end of the season.
...
```{r}
#| label = "viewers_plot"
plot(season_data$viewers, type="l", col="red", lwd=5, xlab = "Episode number", ylab = "1st day TV viewers in the US
    (millions)")
