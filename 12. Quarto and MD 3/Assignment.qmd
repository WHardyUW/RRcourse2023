...
```{r}
num_episodes=nrow(season_data)
average=mean(season_data$viewers)
```

Season 1 of Game of Thrones consisted of `r num_episodes` episodes that aired between April 17 and June 19, 2011 on HBO. The show gathered an average of `r average` first-day TV viewers in the US, with the number growing from 2.22 to 3.04 million by the end of the season.
