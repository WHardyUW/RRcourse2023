...
-   The Q-value is 216.00 with 26 degrees of freedom, and the p-value is less than 0.0001, indicating **significant heterogeneity.**
...
-   The Q-value is 129.28 with 26 degrees of freedom, and the p-value is less than 0.001, indicating **significant heterogeneity.**
...
```{r}
metareg(m.raw,`Neutral.toys`+`Parent.present`+`Setting`+`Country`)

```

The results show QM(df = 4) = 2.9443, p-val = 0.5672, which does not indicate significant moderating effects of neutral toys, parent presence, setting, and country's gender inequality index on effect size.
