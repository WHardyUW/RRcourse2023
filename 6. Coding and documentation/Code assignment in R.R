...
task_data <- read.csv("/Users/hazelzhou/Desktop/RR/RRcourse2023/6. Coding and documentation/Data/onet_tasks.csv")
...
file_path <- "/Users/hazelzhou/Desktop/RR/RRcourse2023/6. Coding and documentation/Data/Eurostat_employment_isco.xlsx"

iscos <- lapply(sheets, function(sheet) {
...
library(dplyr)
totals <- bind_rows(iscos, .id = "ISCO") %>%
  group_by(ISCO) %>%
  summarize(
...
all_isco <- isco1
for (i in 2:9) {
  all_isco <- bind_rows(all_isco, get(paste0("isco", i)) %>% mutate(ISCO = i))
}
