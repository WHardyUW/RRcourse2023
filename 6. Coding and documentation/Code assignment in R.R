...
for (i in 1:9) {
  isco_i <- read_excel("Data\\Eurostat_employment_isco.xlsx", sheet=paste0("ISCO", i))
  isco <- append(isco, isco_i)
}
...
total_Belgium = 0
for(i in 1:9){
  total_Belgium = total_Belgium +isco[i]$Belgium
}

total_Spain = 0
for(i in 1:9){
  total_Spain = total_Spain +isco[i]$Spain
}

total_Poland = 0
for(i in 1:9){
  total_Poland = total_Poland +isco[i]$Poland
}
...
all_data <- rbind(isco[1], isco[2], isco[3], isco[4], isco[5], isco[6], isco[7], isco[8], isco[9])
...
all_data$total_Belgium <- rep(c(total_Belgium), times=9)
all_data$total_Spain <- rep(c(total_Spain), times=9) 
all_data$total_Poland <- rep(c(total_Spain), times=9)
