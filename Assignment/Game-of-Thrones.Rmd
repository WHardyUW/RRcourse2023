...
Here is a graph showing the percentage change in viewership from the previous episode for each season of Game of Thrones:
...
seasons <- c(1, 2, 3, 4, 5, 6, 7, 8)
viewership <- c(2.52, 3.80, 4.97, 6.88, 7.99, 8.89, 10.26, 11.99)
viewership_df <- data.frame(seasons, viewership)

viewership_df <- viewership_df %>%
  mutate(change = (viewership - lag(viewership))/lag(viewership))

ggplot(data = viewership_df) + 
  geom_line(aes(x = seasons, y = change)) + 
  geom_point(aes(x = seasons, y = change)) + 
  xlab("Season") + 
  ylab("Percentage Change in Viewership") + 
  ggtitle("Percentage Change in Viewership by Season")
...
## Observations
The viewership of Game of Thrones increased significantly over its run, peaking at 11.99 million viewers for the series finale.
However, the percentage change in viewership from the previous episode decreased in the later seasons, with a noticeable drop between seasons 5 and 6. Specifically, the viewership decreased by 11.96% between seasons 5 and 6.
Despite the drop in percentage change in viewership, Game of Thrones remained a highly popular and successful show throughout its run.
